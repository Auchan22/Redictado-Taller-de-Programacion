...
procedure GenerarLista(v: vector; dimL: rango3; var L: Lista);
	...
	function esCodigoPar(c: rango1): boolean;
	begin
		esCodigoPar:= c MOD 2 = 0;
	end;
	
	procedure agregarAtras(var pri, ult: Lista; p: venta);
	var
		nue: Lista;
	begin
		new(nue);
...
begin
	for i:= 1 to dimL do begin
		if(esCodigoPar(v[i].codigoP)) then
...
begin
	while(L <> nil) do begin
		writeln('-----');
		writeln('Codigo Producto: ', L^.v.codigoP);
		writeln('Cantidad: ', L^.v.cantidad);
		writeln('-----');
		L:= L^.sig;
	end;
end;
